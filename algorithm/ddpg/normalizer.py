...
class Normalizer_torch:
    def __init__(self, shape, eps_std=1e-2, norm_clip=5):
        self.shape = shape
        self.eps_std = torch.tensor(eps_std)
        self.norm_clip = norm_clip

        self.sum = torch.zeros(self.shape, dtype=torch.float32, requires_grad=False)
        self.sum_sqr = torch.zeros(self.shape, dtype=torch.float32, requires_grad=False)
        self.cnt = torch.zeros(1, dtype=torch.float32, requires_grad=False)
        self.mean = torch.zeros(self.shape, dtype=torch.float32, requires_grad=False)
        self.std = torch.zeros(self.shape, dtype=torch.float32, requires_grad=False)
    ...
    def update(self, inputs):
        ...
        add_cnt = torch.tensor([inputs.shape[0]], dtype=torch.float32)
...
if __name__ == "__main__":
    obs = np.load("silinecek/obs.npy")
    acts = np.load("silinecek/acts.npy")
    obs_next = np.load("silinecek/obs_next.npy")
    #Tensorflow Part
    config = tf.ConfigProto()
    config.gpu_options.allow_growth = True
    sess = tf.Session(config=config)
    obs_normalizer = Normalizer_TF(28, sess)
    raw_obs_ph = tf.placeholder(tf.float32, [None] + [28])
    obs_ph = obs_normalizer.normalize(raw_obs_ph)
    batch = {'obs':obs, 'obs_next':obs_next}
    obs_normalizer.normalizer_update(batch)
    #Pytorch part
    normalizer = Normalizer(28)
    normalizer.normalizer_update(batch)
    print('test')
