...
class FetchEnv(robot_env.RobotEnv):
    ...
    def __init__(
        ...
        distance_threshold, initial_qpos, reward_type,env_name
    ):
        ...
        if self.env_name in ["FetchPickAndPlace-v1", "FetchPickAndPlaceObs-v1"]:
            self.target_range_x = 0.01
            self.target_range_y = 0.0
        elif self.env_name in ["FetchPush-v1", "FetchPushObs-v1"]:
            self.target_range_x = 0.01
            self.target_range_y = 0.0
        elif self.env_name in ["FetchSlide-v1", "FetchSlideObs-v1"]:
            self.target_range_x = 0.0
            self.target_range_y = 0.05
        elif self.env_name in ["FetchReach-v1", "FetchReachObs-v1"]:
            self.target_range_x = 0.01
            self.target_range_y = 0.0
            self.target_range_z = 0.01
        else:
    ...
    def _reset_sim(self):
        ...
        if self.has_object:
            ...
            random_offset = np.random.uniform(-1, 1) * self.obj_range
            if self.env_name == 'FetchSlide-v1':
    ...
    def _sample_goal(self):
        if self.has_object:
            ...
            goal[1] += self.np_random.uniform(-self.target_range_y, self.target_range_y)
            goal[0] += self.np_random.uniform(-self.target_range_x, self.target_range_x)
            goal[2] += 0.1 + self.np_random.uniform(-self.target_range_z, self.target_range_z)
        return goal.copy()
